...
%   产生的新的模型诞生的特征向量，即新的观察值（状态向量）以及获取即使奖励
    ...
    % 更改参数
    cd ..\
    newParam = chengeSL_BLOCKLIBS(Action);      %newParam临时变量，函数末尾可clear newParam
    %disp(newParam);
    cfg_new = cfg(newParam);                     %函数末尾可clear cfg_new
    disp(cfg_new)
    %执行slforge，生成模型
    slforgestart = tic;
    sgtest(cfg_new);
    ...
    if  isempty(successfile)
        ...
        reward = -100;
        restart = true;
        save(fullfile(Nonfile,'Rewardreport.mat'),'totalState','totalFeature','reward','Action','oldstate','new_State','Feature')
...
%     new_State = cat(2,new_State2,succstate);
    ...
    Feature  = new_State;
    totalFeature = cat(1,totalFeature,mean(Feature));
    save(fullfile(successPath,'Rewardreport.mat'),'totalFeature','Action','Feature')
    ...
    %%  清除变量
    vars = {'newParam','cfg_new','reportsneofile','successfile','new_State1','new_State2'};
    clear(vars{:})
